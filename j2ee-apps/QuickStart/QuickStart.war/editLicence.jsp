<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@ taglib uri="http://www.atg.com/taglibs/daf/dspjspTaglib1_1"
	prefix="dsp"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<dsp:page>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Insert title here</title>
<link rel="stylesheet" href="css/bootstrap.min.css">
<link rel="stylesheet" href="css/mycss.css">
</head>
<body>



<div class="col-sm-9">
		<div class="row">
		<div class="col-sm-3">  </div>
		<div class="col-sm-9">
		<!-- column 2 -->	
       <h3> Edit Licence</h3>  
            
       <hr>

<dsp:importbean bean="/quickstart/RepositoryFormHanlder/DriverFormHandler"/>
		<dsp:setvalue bean="DriverFormHandler.repositoryId" paramvalue="id"/>
			    <dsp:getvalueof var="driverId" param="id" vartype="java.lang.String"></dsp:getvalueof>
		
	
<%-- <dsp:valueof bean="DriverFormHandler.repositoryId"> empty</dsp:valueof> --%>
        <dsp:form  method="post">
        
         <div class="form-group">
            <label class="col-lg-3 control-label">Type :</label>
            <div class="col-lg-8">
<%--                <dsp:input class="form-control"  type="text" bean="DriverFormHandler.value.type" /> --%>
               
               
               <div class="form-group">
<!-- 			      <label for="sel1">Select list (select one):</label> -->
<!-- 			      <select class="form-control" id="sel1"> -->
<!-- 			        <option>1</option> -->
<!-- 			        <option>2</option> -->
<!-- 			        <option>3</option> -->
<!-- 			        <option>4</option> -->
<!-- 			      </select> -->
               
               
               <dsp:select bean="DriverFormHandler.value.type" class="form-control" id="sel1"
					    >
					  <dsp:option value="Type A">Type A</dsp:option>
					  <dsp:option value="Type B">Type B</dsp:option>
					  <dsp:option value="Type C">Type C</dsp:option>
				</dsp:select>
					               
            </div>
          </div>
          
           <div class="form-group">
            <label class="col-lg-3 control-label">Number :</label>
            <div class="col-lg-8">
               <dsp:input class="form-control"  type="text" bean="DriverFormHandler.value.num"/>
            </div>
          </div>
          
    
                   
        <dsp:input type="hidden"  bean="DriverFormHandler.repositoryId"  paramvalue="id"/>
         <label class="col-md-3 control-label"></label>
            <div class="col-md-8">
            <div class=" row spacing-row"></div>
              <dsp:input bean="DriverFormHandler.update"  type="submit" class="btn btn-primary" value="Update" />
              
         


<a class="btn btn-default"href="detailDriver.jsp?id=${driverId}">
  Cancel
</a>



                <dsp:input  bean="DriverFormHandler.updateSuccessURL" type="hidden" value="detailDriver.jsp?id=${driverId}"/>
            </div>
        
        



    </dsp:form>

</body>
</html>
</dsp:page>